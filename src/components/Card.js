...
const cardLikeButtonClassName = `place__like-button ${isLiked ? 'place__like-button_liked' : ''}`; 
  ...
  return (
    ...
    <div className="place">
      <button className={cardDeleteButtonClassName} aria-label="delete place" onClick={() => {props.handleCardDelete(props.card)}}></button>
      <img className="place__image" alt="" src={props.card.link} onClick = {() => {props.handleCardClick(props.card)}}/>
      <div className="place__text-container">
        <h2 className="place__title">{props.card.name}</h2>
        <div className = "place__like-container">
          <button className={cardLikeButtonClassName} aria-label="like place" onClick={() => {props.handleCardLike(props.card)}}></button>
